...
export default async function DashboardPage() {
  ...
  const { count: familyRecordsCount } = await supabase
    .from("family_records")
  ...
  const { count: primariaCount } = await supabase
    .from("students")
    .select("*", { count: "exact", head: true })
    .eq("nivel", "Primaria")

  const { count: secundariaCount } = await supabase
    .from("students")
    .select("*", { count: "exact", head: true })
    .eq("nivel", "Secundaria")
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{familyRecordsCount || 0}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{inicialCount || 0}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{studentsCount || 0}</div>
            <p className="text-xs text-muted-foreground">Datos disponibles</p>
          </CardContent>
        </Card>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
              <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
                ...
                <Badge className="bg-blue-100 text-blue-800 text-xs">{inicialCount || 0} estudiantes</Badge>
              </div>
              <p className="text-sm text-blue-700 mb-3">Edades: 3-5 años</p>
            ...
            <div className="bg-green-50 p-4 rounded-lg border border-green-200">
              <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
                ...
                <Badge className="bg-green-100 text-green-800 text-xs">{primariaCount || 0} estudiantes</Badge>
              </div>
              <p className="text-sm text-green-700 mb-3">Grados: 1° - 6°</p>
            ...
            <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
              <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
                ...
                <Badge className="bg-purple-100 text-purple-800 text-xs">{secundariaCount || 0} estudiantes</Badge>
              </div>
              <p className="text-sm text-purple-700 mb-3">Grados: 1° - 5°</p>
